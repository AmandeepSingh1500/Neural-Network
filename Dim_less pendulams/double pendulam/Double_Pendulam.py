...
# def triple_pendulum_ode_scipy(t, y):
#     theta1, omega1, theta2, omega2, theta3, omega3 = y
#     eq1 = omega1
#     eq1_dot = -(g/(3*L)) * np.sin(theta1) \
...
# Analytical solution for comparison (using solve_ivp)
def double_pendulum_ode_dimensionless(tau, y):
    theta1, omega1, theta2, omega2 = y
    dtheta1 = omega1
    domega1 = -2 * np.sin(theta1) \
              - omega2**2 * np.sin(theta1 - theta2) \
              - np.cos(theta1 - theta2) * omega2
    dtheta2 = omega2
    domega2 = -np.sin(theta2) \
              + omega1**2 * np.sin(theta1 - theta2) \
              + np.cos(theta1 - theta2) * domega1
    return [dtheta1, domega1, dtheta2, domega2]



# Initial conditions for solve_ivp
y0 = [np.pi / 5.7, 0.0, np.pi / 15.9, 0.0]  # [theta1, omega1, theta2, omega2, theta3, omega3]
t_span = (0.0, 10.0)
t_eval = np.linspace(t_span[0], t_span[1], 10000)

# Solve using solve_ivp
sol = solve_ivp(double_pendulum_ode_dimensionless, t_span, y0, t_eval=t_eval, method='RK45')
theta1_analytical, theta2_analytical = sol.y[0], sol.y[2]
...
y_data_all = np.stack([theta1_analytical, theta2_analytical], axis=1)
...
class NeuralNetMultiOutput(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Linear(100, 100),
            nn.Tanh(),
            nn.Linear(100, 2)  # Output layer: 3 outputs (theta1, theta2, theta3)
        )
